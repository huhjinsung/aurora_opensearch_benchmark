...
|------|------|
| 작성일 | 2026-04-17 |
| 수행 목적 | 태그리스 파라미터로그의 실시간 분석을 위한 PoC |
...
8개 벤치마크 시나리오에서 **전수 스캔이 필요한 분석 쿼리(시나리오 2~6, 8)에서 OpenSearch가 57~2,304배 빠른 성능**을 보였습니다. 특히 고객의 핵심 분석 요구인 "자계값 + 위치로그 결합 분석"은 MySQL에서는 SQL만으로 수행이 불가능하며, OpenSearch에서는 106ms의 실시간 응답이 가능합니다.
