...
본 PoC는 실시간 로그 데이터에 대해 **Aurora MySQL**과 **Amazon OpenSearch**의 분석 성능을 정량적으로 비교하여, 실시간 로그 분석 워크로드에 적합한 서비스를 선정하는 것을 목적으로 합니다.

### 1.2 배경

현재 시스템에서 발생하는 파라미터로그를 Aurora MySQL의 VARCHAR 컬럼에 JSON 문자열로 저장하고 있습니다. 또한 파라미터 로그 자체를 하나의 컬럼안에 저장하고 있습니다. 이로 인해 실시간 로그 분석 시 `JSON_EXTRACT()` 등으로 JSON 필드를 파싱 후 분석하는 작업이 필요합니다.

### 1.3 목표

기존의 Aurora MySQL에 저장하는 데이터들을 Amazon OpenSearch에 데이터를 Mapping하여 실시간 로그 분석을 진행합니다. 이를 통해 Aurora MySQL과 Amazon OpenSearch간의 성능 차이를 비교합니다.
...
| **스토리지** | Aurora Storage 활용 | EBS gp3 100 GB x 2 노드 |
...
CREATE TABLE tgls_param_logs (
    ...
    tgls_loc_log_larg_ctt    VARCHAR(3000)  COMMENT '위치로그',
...
**샘플 파라미터로그(tgls_prmt_log_val) JSON 구조 (키 a~u):**
...
> **핵심 차이**: MySQL은 분석을 위해 매번 `JSON_EXTRACT()` → 타입 변환 → 전수 스캔이 필요하지만, OpenSearch는 적재 시점에 파싱 완료되어 역인덱스 기반 즉시 검색/집계가 가능합니다.
...
> 📄 **상세 쿼리는 [benchmark_queries.md](./benchmark_queries.md)를 참조하세요.**

#### 시나리오 1: 자계값 비정상 건수 집계
자계최대값(`d`)이 비정상 범위(-1500 미만 또는 0 초과)인 전체 건수를 집계합니다.
...
> **MySQL 한계**: Base64 위치로그를 SQL로 파싱하여 자계 평균 계산 불가능 → 별도 배치 처리 필수

#### 시나리오 3: 역사별/장치별 이상 거래 집계
자계값 비정상인 거래를 역사ID × 장치ID별로 건수 집계합니다.
...
#### 시나리오 6: 장치 상태별 이상률
장치 상태(`u`)별 전체 건수 및 자계 이상 건수를 집계합니다.
...
> **MySQL 우위**: B-Tree 인덱스 활용으로 동등하거나 더 빠른 성능
...
MySQL에서는 **자계값 조건만 적용 가능**하고, Base64 위치로그를 SQL로 파싱하여 자계 평균을 계산하는 것은 불가능합니다. OpenSearch에서는 적재 시점에 위치로그를 파싱하여 `loc_mag_avg` float 필드로 저장하므로, 두 조건을 모두 적용한 실시간 쿼리가 가능합니다. 이것이 현재 배치 플래그 방식을 실시간으로 전환할 수 있는 핵심 근거입니다.
